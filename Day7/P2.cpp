...
struct Vertex {
    ...
    Vertex(int id, string color) :id(id), color(color), quantity(1) {
    }
...
public:
    ...
    int DFS(Vertex v) {
        if (adj[v.id].size() == 0) return v.quantity;

        //cout << v.color << " " << v.quantity << "\n";

        int count = 0;
        for (auto& it : adj[v.id]) {
            count += DFS(it);
        }
        return (count * v.quantity) + v.quantity;
    }
...
int main() {
    ...
    cout << g.DFS(Vertex(bag, color)) - 1 << "\n";
    // g.BFS(Vertex(bag, color));
